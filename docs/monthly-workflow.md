...
- Chase checking and savings CSV exports.
...
Use `|` in `file_match` for alternate filename tokens that identify the same account, for example `3003|Chasenull`.
...
The command also prints a duplicate summary showing raw Chase rows loaded, duplicates removed, and normalized rows kept.
...
- `venmo_activity`: review raw Venmo export activity and whether each row linked to a Chase transaction.
